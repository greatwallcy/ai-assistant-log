...
// 处理队列：主循环中调用，按帧发送，帧间加延时
// ★关键修复：MP3-TF-16P模块内部每10ms处理一次串口，
//  连续指令必须间隔≥20ms，否则前一条被覆盖！(规格书6.2.2第4条)
//  指定文件夹播放(0x0F)需≥40ms(规格书6.2.2第5条)
void Serial_ProcessQueue(void) {
    uint8_t frame[FRAME_LENGTH];
    
    // 每次处理一整帧（FRAME_LENGTH字节），发送后等待
    while (!Serial_QueueIsEmpty(&uart2Queue)) {
        // 检查队列是否有完整一帧
        if (uart2Queue.count < FRAME_LENGTH) break;
        
        // 出队一整帧
        if (!Serial_Dequeue(&uart2Queue, frame, FRAME_LENGTH)) break;
        
        // 阻塞发送整帧（8字节，9600bps≈8.3ms）
        ...
        // ★帧间延时：确保模块处理完当前指令
        // 规格书6.2.2：连续指令间隔≥20ms，指定文件夹播放(0x0F)需≥40ms
        if (frame[3] == 0x0F) {
            Delay_ms(40);  // 指定文件夹播放，模块需要时间锁定文件
        } else {
            Delay_ms(20);  // 其他指令，至少20ms间隔
        }
    }
}
